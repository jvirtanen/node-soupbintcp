...
class Client extends EventEmitter {
  ...
  login(payload, callback) {
    this._connection.send(packet.LOGIN_REQUEST, packet.formatLoginRequest(payload), callback);
  }

  logout(callback) {
    this._connection.send(packet.LOGOUT_REQUEST, callback);
  }

  send(payload, callback) {
    this._connection.send(packet.UNSEQUENCED_DATA, payload, callback);
  }
